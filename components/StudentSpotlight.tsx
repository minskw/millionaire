...
import { IconGraduationCap, IconCheckCircle } from './Icons';
...
export const StudentSpotlight: React.FC<StudentSpotlightProps> = ({ onClose, theme }) => {
  ...
  useEffect(() => {
    if (!isActive || timeLeft <= 0 || isFinished) return;
      ...
      {!isFinished ? (
        <>
          ...
          <div className="flex gap-4 justify-center">
              ...
              <button
                onClick={() => { setIsActive(false); setIsFinished(true); }}
        ...
        <div className="py-4 flex flex-col items-center animate-in fade-in duration-500">
            <div className="mb-6 text-emerald-400">
                <IconCheckCircle className="w-24 h-24" />
            </div>
            <h2 className={`text-2xl font-bold mb-2 uppercase tracking-wide ${theme.textMain}`}>Interaction Complete</h2>
            <p className={`mb-8 ${theme.textMuted} max-w-xs mx-auto`}>
                The student has provided their advice. Good luck!
            </p>
            <button
                onClick={onClose}
                className={`w-full py-4 rounded-xl font-bold text-xl shadow-xl transition-all hover:scale-105 ${theme.btnPrimary}`}
            >
                RETURN TO GAME
            </button>
        </div>
